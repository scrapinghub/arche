...
def test_get_items(mocker, mocked_items, start, count, filters, expected_items):
    ...
    pd.testing.assert_frame_equal(items, expected_items)
...
@pytest.mark.parametrize(
    ...
    [
        (1000, 0, pd.DataFrame(source_items)),
        (1, 0, pd.DataFrame([{"_key": "0", "_type": "NameItem", "name": "Elizabeth"}])),
        (1, 3, pd.DataFrame([{"_key": "3", "_type": "NameItem", "name": "Vivien"}])),
        (4, 2, pd.DataFrame(source_items).iloc[2:].reset_index(drop=True)),
    ],
)
